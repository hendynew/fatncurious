...
class Fatncurious_model_menu extends CI_Model {
	...
	{
		$data = $this->db->query("SELECT menu.*, restoran.* FROM menu , restoran WHERE restoran.KODE_RESTORAN = menu.KODE_RESTORAN AND menu.KODE_RESTORAN = '$resto' and menu.STATUS='1' order by menu.KODE_JENIS_MENU");
		return $data->result();
	}
